...
const createProjectFolder = (projectPath: string) => {
  fs.mkdirSync(projectPath);
};

const createPackageJson = (projectPath: string, projectName: string) => {
...
const createTsConfigJson = (projectPath: string) => {
  fs.writeFileSync(
...
const createSrcFolderAndFiles = (projectPath: string) => {
  ...
  fs.writeFileSync(
    path.join(projectPath, 'src/app.component.ts'),
    `import { Component } from "0injs-framework";\n\nexport class AppComponent extends Component {\n  constructor() {\n    super("#app", "<h1>🎉 Welcome to 0iNJSFramework project!</h1>");\n  }\n}`
  );
};

const installPackages = (projectPath: string) => {
